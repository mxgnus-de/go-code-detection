...
func TestSQLHelloWorldLanguageDetection(t *testing.T) {
	t.Log("Testing SQL Hello World Language Detection")
	code := `SELECT 'Hello world!' text FROM dual;`
	result := detection.DetectLanguage(code, detection.NewDetectionOptions())

	if result.Language != language.LANGUAGE_SQL {
		t.Errorf("Expected language to be %s, but got %s", language.LANGUAGE_SQL, result.Language)
		t.Errorf("Result: %v", result)
	}
}

func TestSQLFizzBuzzLanguageDetection(t *testing.T) {
	t.Log("Testing SQL FizzBuzz Language Detection")
	code := `-- Load some numbers
...
func TestSQLDateManipulationLanguageDetection(t *testing.T) {
	t.Log("Testing SQL Date Manipulation Language Detection")
	code := `-- March 7 2009 7:30pm EST
...
func TestSQLMergeAndAggregateLanguageDetection(t *testing.T) {
	t.Log("Testing SQL Merge and Aggregate Language Detection")
	code := `-- drop tables
...
func TestSQLFibonacciSequenceLanguageDetection(t *testing.T) {
	t.Log("Testing SQL Fibonacci Sequence Language Detection")
	code := `SELECT round ( EXP ( SUM (ln ( ( 1 + SQRT( 5 ) ) / 2)
...
func TestSQLIntegerComparisionLanguageDetection(t *testing.T) {
	t.Log("Testing SQL Integer Comparision Language Detection")
	code := `SELECT to_char(a)||' is less than '||to_char(b) less_than
   FROM test
   WHERE a < b;
